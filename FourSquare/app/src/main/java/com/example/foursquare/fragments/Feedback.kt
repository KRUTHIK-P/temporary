...
class Feedback : Fragment() {
    ...
    override fun onCreateView(
        ...
        feedbackBinding.submitTv.setOnClickListener {
            val getFeedbackRequest = GetFeedbackRequest(feedbackBinding.feedbackTextInputEditText.text.toString())
            val token = SharedPreferencesManager.getInstance(feedbackBinding.root.context).tokenManager.token
            initViewModel()
            if (token != null) {

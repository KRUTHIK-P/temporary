...
class Details : Fragment() {
    ...
    override fun onCreateView(
        ...
        detailsBinding.ratingLyt.setOnClickListener {
            ...
            view.apply {
                ...
                findViewById<TextView>(R.id.submit_tv).setOnClickListener {
                    ...
                    val token = SharedPreferencesManager.getInstance(detailsBinding.root.context).tokenManager.token
                    initViewModel1()
                    if (token != null) {
        ...
        detailsBinding.addReviewTv.setOnClickListener {
            activity?.supportFragmentManager?.beginTransaction()?.replace(R.id.container, AddReview())?.addToBackStack(null)?.commit()
        }
    ...
    private fun addRating(token: String, addRatingRequest: MutableMap<String, Any>) {
        viewModel.addRating(token, addRatingRequest)
    }

    private fun initViewModel1() {
        viewModel = ViewModelProvider(this)[LoginViewModel::class.java]
        viewModel.addRatingObserver()
...
//                                filledStarLytIds[it.rating.toInt()].layoutParams = params
    ...
    private fun getParticularPlace(id: Map<String, String>) {
        viewModel.getParticularPlace(id)
    }

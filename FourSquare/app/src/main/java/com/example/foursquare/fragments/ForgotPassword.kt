...
//                    .show()
                ...
                if (it["message"] == true){
                    if(arguments?.getString("from") == "Login"){
                        activity?.supportFragmentManager?.beginTransaction()?.replace(R.id.container, ForgotPassword1())?.commit()
                    }
                    else {
                        initViewModel()
                ...
                else Toast.makeText(this@ForgotPassword.context, "Please enter correct otp", Toast.LENGTH_SHORT)
                    .show()
            }
        })
    }

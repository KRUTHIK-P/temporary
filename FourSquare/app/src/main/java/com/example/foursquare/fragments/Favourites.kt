...
class Favourites : Fragment() {
    ...
    override fun onCreateView(
        ...
        val token = SharedPreferencesManager.getInstance(favouritesBinding.root.context).tokenManager.token
        initViewModel()
        if (token != null) {
    ...
    private fun SearchView.setOnQueryTextListener() {
        setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String): Boolean {
                val getFavouritesRequest = GetFavouritesRequest(latLangg.latitude, latLangg.longitude, newText)
                val token = SharedPreferencesManager.getInstance(favouritesBinding.root.context).tokenManager.token
                initViewModel()
                if (token != null) {
                    getFavourites(token, getFavouritesRequest)
